...
json_encode_string(A, State) when is_atom(A) ->
    ...
    case json_string_is_safe(L) of
        true ->
            [?Q, L, ?Q];
        false ->
            L_utf8 = binary_to_list(fixutf8:fix_binary(list_to_binary(L))),
            json_encode_string_unicode(xmerl_ucs:from_utf8(L_utf8), State, [?Q])
    end;
json_encode_string(B, State) when is_binary(B) ->
    case json_bin_is_safe(B) of
        true ->
            [?Q, B, ?Q];
        false ->
            B_utf8 = fixutf8:fix_binary(B),
            json_encode_string_unicode(xmerl_ucs:from_utf8(B_utf8), State, [?Q])
    end;
...
json_encode_string(L, State) when is_list(L) ->
    case json_string_is_safe(L) of
        ...
        false ->
            L_utf8 = binary_to_list(fixutf8:fix_binary(list_to_binary(L))),
            json_encode_string_unicode(L_utf8, State, [?Q])
    end.
